...
- **Primary**: Use the `web_fetch` tool for fetching URLs.
- **Fallback**: If `web_fetch` fails (JS-heavy page, bot detection, missing content), use the `playwright` skill:
  - Read `/Users/mhall/Workspaces/nanobot/nanobot/skills/playwright/SKILL.md` for usage instructions.
  - The skill uses Chromium headless to render pages fully before extracting content.
  - Prefer playwright when the page requires JavaScript rendering or when web_fetch returns incomplete/empty content.
